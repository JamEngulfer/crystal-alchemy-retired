...
public class CrystalUpdate {
    ...
    public CrystalUpdate(String modName, String oldVer, String versionURL,
            String updateURL, String[] loadMsg, String[] inGameMsg,
            Logger logger) {
        ...
        try {
            ...
            logger.log(Level.WARNING, "Error initializing ModVersionChecker: "
                    + e.getMessage());
        }
    ...
    public CrystalUpdate(String modName, String oldVer, String versionURL,
            String updateURL, Logger logger) {
        this(
                modName,
                oldVer,
                versionURL,
                updateURL,
                new String[] { "{modName} {oldVer} is out of date! Visit {updateURL} to download the latest release ({newVer})." },
    ...
    public void checkVersionWithLoggingBySubStringAsFloat(int beginIndex,
            int endIndex) {
    ...
    public boolean isCurrentVersionBySubStringAsFloatNewer(int beginIndex,
            int endIndex) {
        return Float.valueOf(newVer.substring(beginIndex, endIndex)) <= Float
                .valueOf(oldVer.substring(beginIndex, endIndex));
    }

    private String replaceAllTags(String s) {
        return s.replace("{oldVer}", oldVer).replace("{newVer}", newVer)
    ...
    private String[] loadTextFromURL(URL url) {
        ...
        try {
            ...
            logger.log(Level.WARNING, "Error getting current version info: "
                    + e.getMessage());
